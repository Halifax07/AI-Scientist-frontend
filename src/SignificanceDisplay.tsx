...
export function SignificanceDisplay({ round }: Props) {
  ...
  const pValue = numberFrom(summary.p_value as number | undefined)
    ?? numberFrom((summary.paired_metric_summaries as Record<string, unknown>)?.p_value);
  const ci = summary.confidence_interval as [number, number] | undefined
    ?? (summary.paired_metric_summaries as Record<string, { ci_95?: [number, number] }>)?.ci_95;

  const effectSize = numberFrom(summary.effect_size as number | undefined)
